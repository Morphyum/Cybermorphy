...
public class HELPER {
	public static String getYoutube(String link) {
		...
		String video = getHTML("https://www.googleapis.com/youtube/v3/videos?part=snippet&id="
				+ id + "&key=AIzaSyDOqZtT01vX5FyWTPzalHIfq-wbOXIju2w");
		JSONObject jsonobject = new JSONObject(video);
		JSONArray items = jsonobject.getJSONArray("items");
		jsonobject = items.getJSONObject(0).getJSONObject("snippet");
		String response = "Oh thanks! A link to the video: "
				+ jsonobject.getString("title") + " uploaded by "
	...
	public static void srlStandings(MessageEvent event) {
		...
		for (int i = 0; i < 10; i++) {
			leader += "#" + (i + 1) + " "
					...
					+ jsonarray.getJSONObject(i).getInt("trueskill") + " --- ";
		}
			...
			if (jsonarray.getJSONObject(i).getString("name")
				...
				event.getBot().sendMessage(
						...
						name
								+ " is currently ranked "
								+ "#"
								+ jsonarray.getJSONObject(i).getInt("rank")
								+ " with "
								+ jsonarray.getJSONObject(i)
										.getInt("trueskill") + " Points");
				match = true;
			}
		}
		if (!match) {
			event.getBot().sendMessage(event.getChannel(),
					name + " is not ranked yet.");
		}
		match = false;
	...
	public static String showSMWCats() {
		...
		for (int h = 0; h < sections.length(); h++) {
			catList += sections.getJSONObject(h).getString("line")
					.toLowerCase()
					+ " | ";
		}
		return catList;
	...
	public static String getWR(String category) {
		...
		for (int h = 0; h < sections.length(); h++) {
			if (sections.getJSONObject(h).getString("line").toLowerCase()
					.contentEquals(category)) {
				catfound = true;
				String section = sections.getJSONObject(h).getString("index");
				String wikitext = getHTML("http://www.deanyd.net/smw/api.php?action=parse&page=leaderboards&format=json&section="
						+ section);
				...
				return ("The World Record for " + category + " is "
						+ wrtime[2].replace("</td>", "").trim() + " by " + wrname[1]
						.replace("</a", "").trim());
			}
		}
		if (!catfound) {
			...
			if (sections.getJSONObject(h).getString("line").toLowerCase()
				...
				for (int i = 0; i < pbtext.length; i++) {
					if (pbtext[i].contains("title")) {
						...
						if (pbhelp[1].replace("</a", "").trim().toLowerCase()
								.contentEquals(name)) {
							playerfound = true;
							return (pbhelp[1].replace("</a", "").trim()
									+ " is currently ranked #" + ranking
									+ " on the " + category
									+ " Leaderboard with a time of " + pbtext[i + 1]
									.replace("</td>", "").trim());
						}
					}
				...
				if (!playerfound) {
					return (name
							+ " was not found on the leaderboard for the category " + category);
				}
				playerfound = false;
				break;
			}
	...
	public static void sendMail(String userMessage) {
		...
		Session session = Session.getDefaultInstance(props,
				new javax.mail.Authenticator() {
					protected PasswordAuthentication getPasswordAuthentication() {
						return new PasswordAuthentication("themorphyum",
								"nudelsuppe");
					}
				});
		...
		try {
			...
			message.setRecipients(Message.RecipientType.TO,
					InternetAddress.parse("themorphyum@gmail.com"));
